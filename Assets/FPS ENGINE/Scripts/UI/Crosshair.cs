...
public class Crosshair : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (InputManager.shooting && player.weapon.canShoot && player.weapon != null)
    ...
    void OnGUI()
    {
        if (player.GetComponent<PlayerStats>().isDead 
                || player.GetComponent<WeaponController>().weapon != null && player.GetComponent<WeaponController>().isAiming && player.GetComponent<WeaponController>().removeCrosshairOnAiming
                || PauseMenu.Instance != null && PauseMenu.isPaused && hideCrosshairOnPaused
                || player.GetComponent<InteractManager>().inspecting && hideCrosshairOnInspecting) return;
        ...
        if (GetComponent<CrosshairShape>().parts.leftPart) GUI.DrawTexture(new Rect((Screen.width / 2 - size / 2) - spread / 2, Screen.height / 2 - width / 2, size, width), texture);
            ...
            if(GetComponent<CrosshairShape>().parts.center)
            {
