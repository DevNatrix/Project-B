...
public class WeaponController : MonoBehaviour
{
    ...
    private void HitscanShot()
    {
        ...
        {
            float dmg = damagePerBullet * GetComponent<PlayerStats>().damageMultiplier;
            ...
            if (Physics.Raycast(newRay, out newHit, penetrationAmount, hitLayer))
            {
                ...
                {
                    float dmg_ = damagePerBullet * GetComponent<PlayerStats>().damageMultiplier * weapon.penetrationDamageReduction;
    ...
    private void ProjectileShot()
    {
        ...
        foreach (var p in firePoint)
        {
            ...
            bullet.damage = damagePerBullet * GetComponent<PlayerStats>().damageMultiplier;
    ...
    private IEnumerator DefaultReload()
    {
        ...
        // Play reload sound
        SoundManager.Instance.PlaySound(weapon.audioSFX.reload, .1f, 0, true, 0);
        reloading = true;
    ...
    public void UnHolster(GameObject weaponObj, bool playAnim)
    {
        ...
        if(playAnim)StartCoroutine(CowsinsUtilities.PlayAnim("unholster", inventory[currentWeapon].GetComponentInChildren<Animator>()));
    ...
    {
        GameObject flashlight = inventory[currentWeapon].flashlight.GetComponent<Flashlight>().lightSource.gameObject;
        inventory[currentWeapon].flashlight.GetComponent<Flashlight>().EnableFlashLight(!flashlight.activeSelf); 
        inventory[currentWeapon].flashlight.GetComponent<Flashlight>().CheckIfCanTurnOn(!flashlight.activeSelf);

        if (flashlight.activeSelf)
            SoundManager.Instance.PlaySound(inventory[currentWeapon].flashlight.GetComponent<Flashlight>().turnOnSFX, 0, 0, true, 0);
        else 
            SoundManager.Instance.PlaySound(inventory[currentWeapon].flashlight.GetComponent<Flashlight>().turnOffSFX, 0, 0, true, 0);
    }
    public void InitializeInspection() => StartCoroutine(CowsinsUtilities.PlayAnim("inspect", inventory[currentWeapon].GetComponentInChildren<Animator>()));

    public void DisableInspection() => StartCoroutine(CowsinsUtilities.PlayAnim("finishedInspect", inventory[currentWeapon].GetComponentInChildren<Animator>()));
